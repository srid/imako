...
spec = do
  ...
  describe "isDailyNote" $ do
    let configNoFolder = DailyNotesConfig Nothing "YYYY-MM-DD"
    let configWithFolder = DailyNotesConfig (Just "Journal") "YYYY-MM-DD"
  ...
  describe "parseDailyNoteDate" $ do
    let config = DailyNotesConfig Nothing "YYYY-MM-DD"
    ...
    it "handles different date formats" $ do
      let euroConfig = DailyNotesConfig Nothing "DD-MM-YYYY"
  ...
  describe "getTodayNotePath" $ do
    ...
    it "generates path with folder" $ do
      let config = DailyNotesConfig (Just "Journal") "YYYY-MM-DD"
      getTodayNotePath config day `shouldBe` "Journal/2025-11-29.md"

    it "handles different date formats" $ do
      let config = DailyNotesConfig Nothing "DD-MM-YYYY"
