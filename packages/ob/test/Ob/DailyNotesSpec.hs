...
spec = do
  ...
  describe "isDailyNote" $ do
    let configNoFolder = DailyNotesConfig "." "YYYY-MM-DD"
    let configWithFolder = DailyNotesConfig "Journal" "YYYY-MM-DD"
    ...
    it "handles current directory notation" $ do
      -- ./path gets normalized to . by takeDirectory, which matches default folder
      isDailyNote configNoFolder "./2025-11-29.md" `shouldBe` True

  describe "parseDailyNoteDate" $ do
    let config = DailyNotesConfig "." "YYYY-MM-DD"
    ...
    it "handles different date formats" $ do
      let euroConfig = DailyNotesConfig "." "DD-MM-YYYY"
  ...
  describe "getTodayNotePath" $ do
    ...
    it "generates path with folder" $ do
      let config = DailyNotesConfig "Journal" "YYYY-MM-DD"
      getTodayNotePath config day `shouldBe` "Journal/2025-11-29.md"

    it "handles different date formats" $ do
      let config = DailyNotesConfig "." "DD-MM-YYYY"
