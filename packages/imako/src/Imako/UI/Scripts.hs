...
appScripts = do
  script_ [] $
    unlines
      ...
      , "}"
      , ""
      , "function togglePastTasks() {"
      , "  const container = document.getElementById('task-content');"
      , "  const btn = document.getElementById('past-tasks-toggle');"
      , "  if (!container || !btn) return;"
      , "  "
      , "  const isShowing = container.classList.toggle('show-past');"
      , "  btn.setAttribute('aria-pressed', isShowing);"
      , "  saveFilterState('showPast', isShowing);"
      , "  hideEmptyContainers();"
      , "}"
      , ""
