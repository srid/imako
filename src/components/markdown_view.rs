...
fn render_block(block: &Block) -> Element {
  match block {
    Block::Paragraph(inlines) => {
      rsx! {
        p { class: "text-stone-800 leading-relaxed mb-5 last:mb-0",
    ...
    Block::BulletList(items) => {
      rsx! {
        div { class: "space-y-1.5 mb-5 last:mb-0",
          for item in items {
            div { class: "flex items-start gap-3 text-stone-800",
    ...
    Block::CodeBlock { language, code } => {
      ...
      rsx! {
        div { class: "my-4 rounded-xl overflow-hidden ring-1 ring-stone-200 bg-stone-50",
          if !lang_label.is_empty() {
            div { class: "px-4 py-1.5 text-xs font-mono text-stone-500 bg-stone-100 border-b border-stone-200",
              "{lang_label}"
            }
          }
          pre { class: "p-4 text-sm font-mono overflow-x-auto text-stone-800",
            code { "{code}" }
          }
        }
      }
    ...
    Block::BlockQuote(children) => {
      rsx! {
        blockquote { class: "border-l-4 border-amber-500 bg-amber-50/50 px-5 py-3 rounded-r-lg my-6 italic text-stone-700 shadow-sm",
    ...
    Block::TaskListItem { checked, content } => {
      ...
      rsx! {
        div { class: "flex items-start gap-2 py-1",
          span { class: "text-stone-400 w-5 h-5 flex-shrink-0",
            "{icon}"
          }
          div { class: "{style}",
...
fn render_inline(inline: &Inline) -> Element {
  match inline {
    ...
    Inline::Code(code) => {
      rsx! {
        code { class: "bg-stone-100 border border-stone-200 px-1.5 py-0.5 rounded-md text-[0.9em] font-medium text-amber-700",
          "{code}"
        }
      }
    }
    ...
    Inline::Strikethrough(children) => {
      rsx! {
        del { class: "text-stone-400 decoration-stone-300",
          InlineRenderer { inlines: children.clone() }
        }
      }
    }
